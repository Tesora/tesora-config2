...
# Cloud credentials are stored in this directory for launch-node.py.
  ...
  file { '/etc/openstack/clouds.yaml':
    ...
    group   => 'admin',
    mode    => '0660',
    content => template('tesora_cyclone/puppetmaster/ansible-clouds.yaml.erb'),
  }
  ...
  file { '/etc/openstack/all-clouds.yaml':
    ...
    group   => 'admin',
    mode    => '0660',
    content => template('tesora_cyclone/puppetmaster/all-clouds.yaml.erb'),
  }
...
# For puppet master apache serving.
  ...
  file { '/etc/apache2/sites-available/puppetmaster.conf':
    ...
    group   => 'root',
    mode    => '0600',
    content => template('tesora_cyclone/puppetmaster/puppetmaster_vhost.conf.erb'),
...
# For launch/launch-node.py.
  ...
  file { '/root/signing.gnupg/gpg.conf':
    ...
    group   => 'root',
    mode    => '0400',
    source  => 'puppet:///modules/tesora_cyclone/puppetmaster/signing.conf',
  ...
  file { '/root/signing.gnupg/sks-keyservers.netCA.pem':
    ...
    group   => 'root',
    mode    => '0400',
    source  => 'puppet:///modules/tesora_cyclone/puppetmaster/sks-ca.pem',
    require => File['/root/signing.gnupg'],
  }
...
# Enable puppetdb
  ...
  file { '/etc/ansible/hosts/infracloud':
    ...
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/tesora_cyclone/puppetmaster/infracloud',
  }
  ...
  file { '/etc/ansible/groups.txt':
    ...
    group   => 'root',
    mode    => '0444',
    source  => 'puppet:///modules/tesora_cyclone/puppetmaster/groups.txt',
  ...
  file { '/usr/local/bin/expand-groups.sh':
    ...
    group  => 'root',
    mode   => '0755',
    source => 'puppet:///modules/tesora_cyclone/puppetmaster/expand-groups.sh',
    notify => Exec['expand_groups'],
  }
