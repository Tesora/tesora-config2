...
class tesora_cyclone::template (
    ...
    'Debian': {
      ...
      if $purge_apt_sources == true {
        file { '/etc/apt/sources.list.d/openstack-infra.list':
          ...
          mode   => '0444',
          owner  => 'root',
          source => "puppet:///modules/tesora_cyclone/sources.list.${::lsbdistcodename}",
        }
        exec { 'update-apt':
  ...
  ###########################################################
  # Manage Puppet
  # possible TODO: break this into tesora_cyclone::puppet
  ...
  if ($::osfamily == 'Debian') {
    ...
    file { '/etc/security/limits.d/60-nofile-limit.conf':
      ...
      group   => 'root',
      mode    => '0644',
      source  => 'puppet:///modules/tesora_cyclone/debian_limits.conf',
      replace => true,
    }
    ...
    file { '/etc/apt/apt.conf.d/90no-translations':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/tesora_cyclone/90no-translations',
      replace => true,
    }
  ...
  if ($::operatingsystem == 'CentOS') {
    if ($::operatingsystemmajrelease == '6') {
      $puppet_repo_source_path =
        'puppet:///modules/tesora_cyclone/centos6-puppetlabs.repo'
      $custom_cgit = present
    } elsif ($::operatingsystemmajrelease == '7') {
      $puppet_repo_source_path =
        'puppet:///modules/tesora_cyclone/centos7-puppetlabs.repo'
      $custom_cgit = absent
    }
    ...
    file { '/etc/yum.repos.d/git-1237395.repo':
      ...
      group   => 'root',
      mode    => '0444',
      source  => 'puppet:///modules/tesora_cyclone/git-1237395.repo',
      replace => true,
    }
